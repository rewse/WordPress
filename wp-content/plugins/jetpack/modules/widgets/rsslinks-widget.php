...
class Jetpack_RSS_Links_Widget extends WP_Widget {
	...
	function _rss_link( $type = 'posts', $args ) {
		...
		if ( 'image' == $format || 'text-image' == $format )
			$link_item = '<a target="' . $link_target . '" href="' . get_bloginfo($rss_type) . '" title="' . esc_attr( $subscribe_to ) . '"><img src="' . esc_url( plugins_url( 'images/rss/' . $args['imagecolor'] . '-' . $args['imagesize'] . '.png', dirname( dirname( __FILE__ ) ) ) ) . '" alt="RSS Feed" width="14px" height="14px"/></a>';
